...
class WeatherService {
    ...
    static func getWeatherData(lat: Float, lon: Float,time: Int, completionHandler: @escaping (_ data: Data?, _ response: URLResponse?, _ error: Error?) -> Void) {
        guard let url = URL(string: "https://api.openweathermap.org/data/2.5/onecall/timemachine?lat=\(lat)&lon=\(lon)&dt=\(time)&units=metric&exclude=minutely,hourly,daily,icon&appid=\(self.key)") else {
            return
        }
